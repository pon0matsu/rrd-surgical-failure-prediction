# %%
# Install the pinned dependencies from ../requirements.txt before running.
import csv
import os
...
# The original notebook contained this Python source in a cell that was
# accidentally marked as Markdown. Restore the submitted TabPFN 5-fold
# fit/predict workflow here without changing its model configuration.
output_dir = 'analysis_outputs'
tabpfn_output_dir = os.path.join(output_dir, 'CV_5FOLD_ORIGINAL', 'tabpfn')
os.makedirs(tabpfn_output_dir, exist_ok=True)
...
for fold in range(5):
    train_data = pd.read_csv(
        os.path.join(output_dir, f'fold_{fold}_train_data_after_imputation.csv')
    )
    val_data = pd.read_csv(
        os.path.join(output_dir, f'fold_{fold}_val_data_after_imputation.csv')
    )
    ...
    val_data.to_csv(
        os.path.join(
            tabpfn_output_dir,
            f'Results_ORIGINAL_fold_{fold}_val_data_with_pred_proba.csv',
        ),
        index=False,
    )
    ...
    plt.plot(
        fpr,
        tpr,
        label=f'Fold {fold} (AUC = {roc_auc_scores[-1]:.2f})',
    )
...
plt.savefig(
    os.path.join(tabpfn_output_dir, 'roc_curve_fold_tabpfn.png'),
    dpi=300,
)
plt.show()
...
# Create the CSV file
with open(os.path.join(tabpfn_output_dir, 'evaluation_results_5fold_tabpfn.csv'), mode='w', newline='') as file:
