...
from review_common import BOOTSTRAP_ITERATIONS, BOOTSTRAP_SEED, SUBMITTED_CODE_ROOT, TARGET_COLUMN, wide_metric_summary  # noqa: E402
...
PREDICTION_EXPORT_INDEX_CSV = PRIVATE_PREDICTION_EXPORT_DIR / "pvd_only_prediction_export_index.csv"
...
ANALYSIS_NAME = "pvd_only_rebuild"
PRIMARY_FEATURE_SET = "submitted_rfecv36"
HOLDOUT_DATASET_NAME = "pvd_related_ppv_holdout"
CV_FOLDS = 5
...
def load_prepared_train_holdout() -> tuple[pd.DataFrame, pd.DataFrame]:
    if not TRAIN_AFTER_CSV.is_file() or not HOLDOUT_AFTER_CSV.is_file():
        raise FileNotFoundError("Run 42_01_prepare_pvd_only_training_data.py before this script.")
...
def load_fold_train_validation(fold_index: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    train_path = PRIVATE_FOLD_DIR / f"pvd_only_rfecv36_fold_{fold_index}_train_data_after_imputation.csv"
    validation_path = PRIVATE_FOLD_DIR / f"pvd_only_rfecv36_fold_{fold_index}_val_data_after_imputation.csv"
...
def load_fold_undersampled_train(fold_index: int) -> pd.DataFrame:
    path = PRIVATE_FOLD_DIR / f"pvd_only_fold_{fold_index}_undersampled_train_data.csv"
    if not path.is_file():
        raise FileNotFoundError("Run 42_01_prepare_pvd_only_training_data.py before this script.")
...
def load_final_undersampled_train() -> pd.DataFrame:
    path = PRIVATE_DATA_DIR / "pvd_only_train_data_after_imputation_undersampled.csv"
    if not path.is_file():
        raise FileNotFoundError("Run 42_01_prepare_pvd_only_training_data.py before this script.")
    return pd.read_csv(path, low_memory=False, encoding="utf-8-sig")
...
def align_to_submitted_prediction_schema(export: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    if missing or extra:
        raise ValueError("PVD-only prediction export does not match the submitted pred_proba column schema.")
...
def run_analysis() -> None:
    ...
    print(f"[{STEP_DIR.name}] Step 42-02 model rebuild: TabPFN device={TABPFN_DEVICE}, ensembles={TABPFN_ENSEMBLES}", flush=True)
    ...
    step05_tuning = pd.concat(
        [
            run_step05_grid_search(train_after, rfecv_features, use_random_undersampling=False),
            run_step05_grid_search(train_after, rfecv_features, use_random_undersampling=True),
        ],
        ignore_index=True,
    )
    ...
    step07_metrics, step07_status = run_step07_cv_original(rfecv_features, best["no_undersampling"])
    step08_metrics, step08_status = run_step08_cv_undersampling(rfecv_features, best["random_undersampling"])
