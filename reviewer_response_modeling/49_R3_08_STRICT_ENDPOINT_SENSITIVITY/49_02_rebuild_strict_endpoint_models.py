"""Step 49-02: rebuild strict-endpoint models from prepared data.

Run `49_01_prepare_strict_endpoint_training_data.py` first.

This file keeps the model-building code close to the submitted notebooks:
...
The endpoint has changed, so LR/RF/XGB/LGB are retuned with the submitted Step
05 grid on the changed training set.  TabPFN is not grid-tuned, consistent with
the submitted notebooks.

Public outputs are limited to hyperparameter tuning, combined CV/hold-out
metrics, and model run status. Private data files generated by Step 49-01 are
used for fitting but are not duplicated as public file-index outputs.
"""
...
def run_analysis() -> None:
    ...
    # 2. Re-run Step05-style hyperparameter tuning on the strict-endpoint target.
